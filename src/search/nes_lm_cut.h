...
#ifndef NES_LM_CUT_H_
#define NES_LM_CUT_H_
...
class NesLandmarkCutHeuristic : public Heuristic {
   ...
   // std::vector<std::pair<int,int> > goal;
    ...
    RelaxedOperator get_stubborn_op(const Operator* stub_op);
...
public:
    ...
    NesLandmarkCutHeuristic(const Options &opts);
    virtual ~NesLandmarkCutHeuristic();
    int compute_a_nes(const State &state, const Operator* stub_op);
    std::vector<const Operator*> nes; // necessary enabling set
   std::vector<std::vector<const Operator*> > all_cuts;
   // std::vector<std::vector<const Operator*> > all_nes;
    int por_compute_heuristic(const State &state);
    void initialize_nes();

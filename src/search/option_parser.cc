...
static void get_help(string k) {
    ...
    get_help_templ<GraphCreator *>(pt);
...
static void get_full_help() {
    ...
    get_full_help_templ<GraphCreator *>();
    Plugin<OpenList<int> >::register_open_lists();
...
static void predefine_symmetries(std::string s, bool dry_run) {
    ...
    OptionParser op(rs, dry_run);
    if (definees.size() == 1) {
        Predefinitions<GraphCreator *>::instance()->predefine(
            definees[0], op.start_parsing<GraphCreator *>());
    } else {
        op.error("predefinition has invalid left side");
    }
}
...
SearchEngine *OptionParser::parse_cmd_line(
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg.compare("--heuristic") == 0) {
            ...
            ++i;
            predefine_symmetries(argv[i], dry_run);

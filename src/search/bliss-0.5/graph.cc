			   ...
			   Stats &stats)
{
  ...
  while(!search_stack.empty()) 
    ...
    // print the cells
    /*
         for(Partition::Cell *cell11 = p.first_nonsingleton_cell; cell11; cell11 = cell11->next_nonsingleton)
  {
    
	  int min = p.elements[cell11->first];
	  cout<<"[";
	  for(int idx = cell11->first; idx < cell11->first+cell->length; idx++){
	    cout<<p.elements[idx]<<" ";
	  }
	  cout<<"]";
 }  	 
    */
	...
	//  cout<<"About to start splitting:(first path)"<<endl;
	 ...
	 // cout<<"Chose "<<next_split_element<<endl;
	}
      else if(info.in_best_path)
	{
	 // cout<<"About to start splitting: best path "<<endl;
	  /* Find the next larger splitting element that is a mor */
	  for(unsigned int i = cell->length; i > 0; i--, ep++) {
	 // cout<<(int)(*ep)<<" ";
	    if((int)(*ep) > info.split_element &&
	...
	{
//	  cout<<"About to start splitting: (else)"<<endl;
...
//	  cout<<endl;
      ...
      if(info.equal_to_first_path)
	{
	  ...
	  // Added by Michael Katz
	  if(verbstr && verbose_level >= 1) {
		  fprintf(verbstr, "Generator %d added in %.2f seconds, time bound: %d, generators bound: %d\n",
				  stats.nof_generators, t1.get_duration(), time_bound, generators_bound);
	        fflush(verbstr);
	  }

	  // Exiting after the time bound in reached
	  if (time_bound && t1.get_duration() > time_bound)
		  break;
	  // Exiting after generators bound is reached
	  if (generators_bound && stats.nof_generators >= generators_bound)
		  break;
...
void Graph::reverse_colors(){
    for (int i = 0; i < vertices.size(); i++){
  //  std::cout<<"before: "<<vertices[i].color<<std::endl;
        vertices[i].color = (99999 - vertices[i].color); // AVIV DONT KILL ME PLEASE!!!
    //std::cout<<"After: "<<vertices[i].color<<std::endl;
    }
...
std::vector<unsigned  int>* Graph::get_edges(int index){
   // std::cout<<vertices[index].edges.size()<<std::endl;
    
    return &(vertices[index].edges);
}
/**
* returns edges, TODO: In my case maybe all the graphs have the same
* number of edges so we can avoid counting them again
*/
unsigned short int* Graph::getEdges() {
 /* remove_duplicate_edges();
  sort_edges();
...
/*     os<< "p edge ";
  ...
  /*for(unsigned int i = 0; i < get_nof_vertices(); i++)
    ...
    //TODO I think I can assume these were called already, but I'm not sure
...
//  sort_edges();
  ...
  unsigned short int* edges = new unsigned short int[nof_edges*2+1];//first element will be size
  edges[0] = nof_edges*2+1;
  int loc = 1; 
  for(unsigned int i = 0; i < get_nof_vertices(); i++)
    {
      Vertex &v = vertices[i];
...
Partition::Cell *Graph::find_next_cell_to_be_splitted(Partition::Cell *cell)
{
  ...
  case shs_flm:
    return sh_first_largest_max_neighbours(cell);
  case shs_nirp:
    return sh_one_with_min_value(cell);
  case shs_nirp2: 
    return sh_min_from_set(cell);
  default:
    assert(false && "Unknown splitting heuristics");
  }
}
...
/** \internal
 * A splitting heuristic. By Nir
 * Returns the non singelton cell with the minimum member
 * Assumes that the partition p is equitable.
 * Assumes that the max_ival fields of the cells are all 0.
 * (nirp - I have no ideas what these assumptions are)
 */
Partition::Cell *Graph::sh_one_with_min_value(Partition::Cell *cell)
{
  Partition::Cell *best_cell = 0;
  int smallest_value = 999999999; //nirp assumes no more vertices than this num...
  for(cell = p.first_nonsingleton_cell; cell; cell = cell->next_nonsingleton)
  {
      BLISS_ASSERT(cell->length > 1);
	  int min = p.elements[cell->first];
	  for(int idx = cell->first; idx < cell->first+cell->length; idx++){
	    if (min<p.elements[idx]){
	        min = p.elements[idx]; //nirp assuming the idx is the vertex number
	    }
	  }
	 
  
      if(smallest_value > min)
	  {	 
	    smallest_value = min;
	    best_cell = cell;
	  }
...
/** \internal
 * A second splitting heuristic. By Nir
 * Returns the non singelton cell with the first number from list of numbers
 * if not then cell with smallest number
 * Assumes that the partition p is equitable.
 * Assumes that the max_ival fields of the cells are all 0.
 * (nirp - I have no ideas what these assumptions are)
 */
Partition::Cell *Graph::sh_min_from_set(Partition::Cell *cell)
{
  // assumes set has been set :)
  ...
  for(cell = p.first_nonsingleton_cell; cell; cell = cell->next_nonsingleton)
  {
      BLISS_ASSERT(cell->length > 1);
	  int min = p.elements[cell->first];
	  for(unsigned int idx = cell->first; idx < cell->first+cell->length; idx++){
	    // if its from the set its the result
	    for (int i = 0; i < length; i++){
	        if (set[i]==p.elements[idx]){
	            return cell;
	        }
	    }
	    if (min<p.elements[idx]){

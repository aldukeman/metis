...
PorEagerSearch::PorEagerSearch(const Options &opts) :
	SearchEngine(opts), reopen_closed_nodes(opts.get<bool> ("reopen_closed")),
			do_pathmax(opts.get<bool> ("pathmax")), use_multi_path_dependence(
					opts.get<bool> ("mpd")), open_list(opts.get<OpenList<
					state_var_t *> *> ("open")), f_evaluator(opts.get<
...
void PorEagerSearch::initialize() {
	//TODO children classes should output which kind of search
	cout << "Conducting best first search" << (reopen_closed_nodes ? " with"
			: " without") << " reopening closed nodes, (real) bound = "
			<< bound << endl;
	if (do_pathmax)
...
void PorEagerSearch::statistics() const {
	search_progress.print_statistics();
	search_space.statistics();
}


int PorEagerSearch::step() {
	...
	// ************************************************************************************
	// planning with an ample set
	std::vector<const Operator *> ample;
	ample = ample_set.computeAmpleSet(s);
	for (int i = 0; i < ample.size(); i++) {
		const Operator *op = ample[i];
		// ************************************************************************************
		//
		...
		if (use_multi_path_dependence || succ_node.is_new()) {
			...
			for (size_t i = 0; i < heuristics.size(); i++)
				h_is_dirty = h_is_dirty || heuristics[i]->reach_state(s, *op,
						succ_node.get_state());
...
pair<SearchNode, bool> PorEagerSearch::fetch_next_node() {
...
void PorEagerSearch::reward_progress() {
	// Boost the "preferred operator" open lists somewhat whenever
	// one of the heuristics finds a state with a new best h value.
	open_list->boost_preferred();
}

void PorEagerSearch::dump_search_space() {
	search_space.dump();
}

void PorEagerSearch::update_jump_statistic(const SearchNode &node) {
	if (f_evaluator) {
...
void PorEagerSearch::print_heuristic_values(const vector<int> &values) const {
...
static SearchEngine *_parse(OptionParser &parser) {
	...
	PorEagerSearch *engine = 0;
	if (!parser.dry_run()) {
		opts.set<bool> ("mpd", false);
		engine = new PorEagerSearch(opts);
	}
...
static Plugin<SearchEngine> _plugin("por_eager", _parse);
static Plugin<SearchEngine> _plugin_astar("por_astar_lm", _parse_astar);
static Plugin<SearchEngine> _plugin_greedy("por_eager_greedy", _parse_greedy);

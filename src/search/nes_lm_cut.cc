...
NesLandmarkCutHeuristic::NesLandmarkCutHeuristic(const Options &opts) :
	Heuristic(opts) {
	num_propositions = 2;
	//initialize();// artifical goal and artificical precondition
}

NesLandmarkCutHeuristic::~NesLandmarkCutHeuristic() {
}


void NesLandmarkCutHeuristic::initialize_nes(){
	initialize();
}
// initialization
void NesLandmarkCutHeuristic::initialize() {
...
void NesLandmarkCutHeuristic::enqueue_if_necessary(RelaxedProposition *prop,
		int cost) {
	assert(cost >= 0);
...
//void NesLandmarkCutHeuristic::set_goal() {
//
//	for (int p = 0; p < op_goal->get_pre_post().size(); p++) {
//		std::pair<int, int> subgoal;
//		subgoal.first = op_goal->get_pre_post()[p].var;
//		subgoal.second = op_goal->get_pre_post()[p].pre;
//		goal.push_back(subgoal);
//	}
//	for (int p = 0; p < op_goal->get_prevail().size(); p++) {
//		std::pair<int, int> subgoal;
//		subgoal.first = op_goal->get_prevail()[p].var;
//		subgoal.second = op_goal->get_prevail()[p].prev;
//		goal.push_back(subgoal);
//	}
//}
void NesLandmarkCutHeuristic::build_relaxed_operator(const Operator &op) {
	...
	for (int i = 0; i < pre_post.size(); i++) {
		...
		if (pre_post[i].pre != -1)
			precondition.push_back(
					&propositions[pre_post[i].var][pre_post[i].pre]);
...
void NesLandmarkCutHeuristic::add_relaxed_operator(const vector<
		RelaxedProposition *> &precondition,
		const vector<RelaxedProposition *> &effects, const Operator *op,
		int base_cost) {
...
void NesLandmarkCutHeuristic::setup_exploration_queue() {
...
void NesLandmarkCutHeuristic::setup_exploration_queue_state(const State &state) {
	for (int var = 0; var < propositions.size(); var++) {
...
void NesLandmarkCutHeuristic::first_exploration(const State &state) {
...
void NesLandmarkCutHeuristic::first_exploration_incremental(vector<
		RelaxedOperator *> &cut) {
	assert(priority_queue.empty());
...
void NesLandmarkCutHeuristic::second_exploration(const State &state, vector<
		RelaxedProposition *> &second_exploration_queue, vector<
		RelaxedOperator *> &cut) {
...
void NesLandmarkCutHeuristic::mark_goal_plateau(RelaxedProposition *subgoal) {
...
void NesLandmarkCutHeuristic::validate_h_max() const {
#ifndef NDEBUG
...
RelaxedOperator NesLandmarkCutHeuristic::get_stubborn_op(
		const Operator* stub_op) {
	RelaxedOperator &op = relaxed_operators[0];
	for (int i = 0; i < relaxed_operators.size(); i++) {
		RelaxedOperator &op = relaxed_operators[i];
		if (stub_op == op.op) {
			return op;
...
int NesLandmarkCutHeuristic::compute_a_nes(
		const State &state, const Operator* stub_op) {
	for (int i = 0; i < relaxed_operators.size(); i++) {
		RelaxedOperator &op = relaxed_operators[i];
	...
	RelaxedOperator relaxed_stub_op = get_stubborn_op(stub_op);
	if (relaxed_stub_op.unsatisfied_preconditions > 0){
	//	cout << "dead end" << endl;
			return DEAD_END;
		}
	int num_iterations = 0;
	while (relaxed_stub_op.h_max_supporter_cost != 0) {

		num_iterations++;
		mark_goal_plateau(relaxed_stub_op.h_max_supporter);
		assert(cut.empty());
...
int NesLandmarkCutHeuristic::compute_heuristic(const State &state) {
...
int NesLandmarkCutHeuristic::por_compute_heuristic(const State &state) {
	//	// TODO: Possibly put back in some kind of preferred operator mechanism.
		for (int i = 0; i < relaxed_operators.size(); i++) {
			RelaxedOperator &op = relaxed_operators[i];
			op.cost = op.base_cost * COST_MULTIPLIER;
...
static ScalarEvaluator *_parse(OptionParser &parser) {
	...
	if (parser.dry_run())
		...
		return new NesLandmarkCutHeuristic(opts);
}

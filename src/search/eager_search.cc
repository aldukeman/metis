...
EagerSearch::EagerSearch(
    ...
    : SearchEngine(opts),
      ...
      num_por_probes(opts.get<int>("num-por-probes")),
      pruning_ratio(opts.get<double>("required-pruning-ratio")),
      conservative_otf(opts.get<bool>("conservative-otf")),
    ...
    partial_order_reduction_method = POR::create(
        opts.get_enum("partial_order_reduction"),
        opts.get<bool>("on-the-fly-interference"),
        opts.get<bool>("on-the-fly-achievers"),
        opts.get<int>("por-mem-bound"),
        opts.get<bool>("conservative-otf"));
    if(opts.get_enum("partial_order_reduction") > 0) {
        por_is_on = true;
    } else {
        por_is_on = false;
    }
...
#ifdef CANONICAL
        cerr << "No symmetry type is defined, and yet additional buffer is allocated per state. If you do not wish to use symmetries, please recompile with CANONICAL=0" << endl;
        exit_with(EXIT_INPUT_ERROR);
#endif
    }
}

State EagerSearch::get_initial_state() {
    if (do_orbit_search) {
        StateRegistry tmp_registry;
        const State &tmp_initial_state = tmp_registry.get_initial_state();
...
int EagerSearch::step() {
    ...
    if(por_is_on && num_por_probes <= search_progress.get_expanded() &&
    !partial_order_reduction_method->sufficient_por_pruning(pruning_ratio)) {
        por_is_on = false;
    }
    ...
    for (int i = 0; i < applicable_ops.size(); i++) {
        ...
        */
        StateRegistry tmp_registry;
        StateRegistry *successor_registry = do_orbit_search? &tmp_registry : g_state_registry;
        State real_succ_state = successor_registry->get_successor_state(s, *op);
        State succ_state = real_succ_state;
        if (do_orbit_search) {
            succ_state = g_state_registry->get_canonical_representative(real_succ_state);
        }
        ...
        if (use_multi_path_dependence || succ_node.is_new()) {
            ...
            for (size_t i = 0; i < heuristics.size(); ++i) {
                ...
                */
                 ...
                 */
                if (heuristics[i]->reach_state(s, *op, real_succ_state))
                    h_is_dirty = true;
            }
        ...
        if (succ_node.is_new()) {
             ...
             */
            for (size_t i = 0; i < heuristics.size(); i++)
                heuristics[i]->evaluate(real_succ_state);

...
/*
  ...
  state_var_t*
    The actual state data is internally represented as a state_var_t array.
...
public:
    ...
    */
    ...
    /*
      Creates the canonical representative of the given State (which can be
      registered somewhere else). Registers and returns the representative
      if this was not done before. This is an expensive operation as it includes
      duplicate checking.
    */
    State get_canonical_representative(const State &state);
    ...
    /*
      ...
      checking.
    */
    State get_state_permutation(const State &state, Permutation &permutation);

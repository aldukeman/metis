...
void LandmarkCutHeuristic::reset_operator_costs(const State &/*state*/) {
...
int LandmarkCutHeuristic::compute_heuristic(const State &state) {
    ...
    while (artificial_goal.h_max_cost != 0) {
        ...
        //cout << "{" << cut_cost << "}" << flush;
        total_cost += cut_cost;
        discovered_landmark(state, cut, cut_cost);
...
static Heuristic *_parse(OptionParser &parser) {
    ...
    if (parser.dry_run())
        ...
        return new conditional_effects::LandmarkCutHeuristic(opts);
}


static Plugin<Heuristic> _plugin("conditional_effect_lmcut", _parse);
